...
def test_guided_v2():
    ...
    for name, cls in sorted(GUIDED_INPAINTERS.items()):
        ...
        check(f'{name}: dead-region posterior', abs(m - MU0) < 0.15 * SIGMA0
              and 0.85 < s_ / SIGMA0 < 1.12,
    ...
    # K-image batched path
    K = 2
    truths = MU0 + SIGMA0 * torch.randn(
        K, 1, H, W, generator=torch.Generator().manual_seed(3))
    outK = GUIDED_INPAINTERS['pigdm2'](net, sc, DEVICE, seed=5).inpaint(
